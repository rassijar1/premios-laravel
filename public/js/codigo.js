  		...
  		if(result.value){
  			...
  			$.ajax({
		        ...
		        error: function (jqXHR, textStatus, errorThrown) {
		          swal({
		                    type:"error",
		                    title: "¡El registro no se puede eliminar porque tiene dependencias con otras tablas!",
		                    showConfirmButton: true,
		                    confirmButtonText: "Cerrar"
...
/*=============================================
Evitar repetir fecha y hora 
=============================================*/
$(document).on("change",".inputHora", function(){

	$(".alert").remove();

	var valorHora = $(this).val();
	var valorFecha = $("#fecha_cita").val();
	var validarHora = $(".validarHora");
	var validarFecha = $(".validarFecha");

	for(var i = 0, j=0; i < validarHora.length, j < validarFecha.length; i++, j++){

		 if(($(validarHora[i]).html() == valorHora)&&($(validarFecha[j]).html() == valorFecha) ){
		 	$(".inputFecha").val("");
		 	 $(".inputHora").val("");
		 	 $(".inputHora").parent().after(`

				<div class="alert alert-danger">¡Error! No tiene permitido Registrar 2 citas en el mismo horario</div>	
...
/*$(document).on("change",".inputFecha", function(){
	...
	for(var i = 0; i < validarFecha.length; i++){

		 if($(validarFecha[i]).html() == valorFecha){

		 	 $(".inputFecha").val("");
		 	 $(".inputFecha").parent().after(`

				<div class="alert alert-danger">¡Error! Esta Fecha ya existe en la base de datos</div>	
